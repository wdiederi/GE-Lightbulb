...
for iY = 1:length(Yrange)
    ...
    if handles.capResistanceButton.Value == 1
        lowerBound = [0.0125;0.006;.007;.007];
    else
        lowerBound = [0;0;0;0];
    end
    
    upperBound = [.075;.06;.077;.077];
    
    % Within 10,000 ohms
    Results = lsqlin(C,D,[],[],[],[],lowerBound, upperBound,[],options);
...
handles.redCurrentText.String = round(1000* LEDans{1}.current, 1);
handles.greenCurrentText.String = round(1000*LEDans{2}.current, 1);
handles.blueCurrentText.String = round(1000*LEDans{3}.current, 1);
handles.whiteCurrentText.String = round(1000*LEDans{4}.current, 1);